...
public class bl_HudDamageManager : MonoBehaviour {
    ...
    public static bl_HudDamageManager instance;
    ...
    public void OnDamage(bl_DamageInfo info)
    {
        //Calculate health
        Health -= info.Damage;
        AudioSource.PlayClipAtPoint(hurtSound, player.transform.position, 0.6f);
    ...
    public IEnumerator Shake()
    {
        ...
        {
            ShakeObject.localPosition = ShakeObject.transform.position + Random.insideUnitSphere* shakeIntensity;
            ShakeObject.localRotation = new Quaternion(
                ShakeObject.rotation.x + Random.Range(-shakeIntensity, shakeIntensity) * ShakeAmount,
                ShakeObject.rotation.y + Random.Range(-shakeIntensity, shakeIntensity) * ShakeAmount,
                ShakeObject.rotation.z + Random.Range(-shakeIntensity, shakeIntensity) * ShakeAmount,
                ShakeObject.rotation.w + Random.Range(-shakeIntensity, shakeIntensity) * ShakeAmount);
            shakeIntensity -= ShakeDecay;
            yield return false;
        }
        ShakeObject.localPosition = ShakeObject.transform.position;
        ShakeObject.localRotation = ShakeObject.rotation;
    }
